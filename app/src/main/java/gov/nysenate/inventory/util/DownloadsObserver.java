...
public class DownloadsObserver extends FileObserver {
    ...
    private static final int flags = FileObserver.CLOSE_WRITE
            | FileObserver.OPEN | FileObserver.MODIFY | FileObserver.DELETE
            | FileObserver.MOVED_FROM;

    // Received three of these after the delete event while deleting a video
    // through a separate file manager app:
    // 01-16 15:52:27.627: D/APP(4316): DownloadsObserver: onEvent(1073741856,
    // null)
    ...
    public void onEvent(int event, String path) {
        ...
        double bytes_total = cursor.getDouble(cursor
                .getColumnIndex(DownloadManager.COLUMN_TOTAL_SIZE_BYTES));

        double dl_progress = (bytes_downloaded / bytes_total) * 100;
        Log.d(LOG_TAG, "onEvent(" + event + ",  PROGESS: (" + bytes_downloaded
